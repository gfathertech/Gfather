...
// Get Replit-specific URL parameters
const replUrl = process.env.REPLIT_PROJECT_DOMAIN 
  ? `https://${process.env.REPLIT_PROJECT_DOMAIN}`
  : `http://0.0.0.0:${PORT}`;
...
// Start server properly for Replit
app.listen(PORT, '0.0.0.0', () => {
  console.log(`Server running at ${replUrl}`);
});
  ...
  async function startBotz() {
    ...
    Gfather.ev.on('connection.update', (update) => {
      ...
      if (connection === 'close') {
        ...
        console.log('[Connected] ' + JSON.stringify(Gfather.user.id, null, 2));

// Improved keep-alive system
setInterval(async () => {
  try {
    const response = await axios.get(`${replUrl}/ping`);
    console.log(`Keep-alive success (${response.status})`);
  } catch (error) {
    console.error('Keep-alive failed:', error.message);
  }
}, 80_000); // 3 minutes interval
